...
// Set up a route to handle comments
router.use("/comments", protect, async (req, res) => {
  const { method, body, headers, originalUrl } = req; // Destructure the request object
  const url = `${BLOG_SERVICE_URL}${originalUrl}`; // Construct the full URL for the Blog Service
  try {
    ...
    res.status(response.status).json(response.data); // Send the response from the Blog Service back to the client
  } catch (error) {
    logger.error(
      `Error forwarding ${method} request to Blog Service: ${url}`,
      error.message
